...
public class ChestSO : ScriptableObject
{
    ...
    public void MoveItemToInventory(int ChestIndex, int InventoryIndex)
    {
        Debug.Log(InventoryIndex);
        InventoryItem item =  Inventory[InventoryIndex];
        InventoryItem item1 = inventorySo.Inventory[ChestIndex];
        if (!item.IsEmpty)
        {
            inventorySo.Inventory[ChestIndex] = item;
            Inventory[InventoryIndex] = item1;
            inventorySo.OnInventoryStateChange();
            OnInventoryStateChange();
        }
    }
    ...
    public void OnInventoryStateChange() { OnInventoryChange?.Invoke(GetChestState()); }


    public Dictionary<int, InventoryItem> GetChestState()
    {
